...
class GetCartUseCaseTest {
    ...
    private val cardRepository: ICartRepository = mock()
    private val productRepository: IProductRepository = mock()

    @Before
    fun init() {
        useCase = GetCartUseCase(cardRepository, productRepository)
    }
    ...
    fun `test get cart items`() = runTest {
        ...
        whenever(cardRepository.getAllCartItems()).thenReturn(cartItems)
